...
def get_em_data(*, read_blockchain: bool = False):
    ...
    if read_blockchain:  # Will use the moralis APIs to query the blockchian.  Takes 10 sec or so.
        ...
        em_data['redemption_queue'] = 1.44E6
        em_data['trunk_supply'] = 30.961E6
        em_data['staking_balance'] = 8.883E6
        stampede_bonds = 56.15E6
        stampede_payouts = 60.12E6
        em_data['farm_tvl'] = 7.535E6  # Yield is paid out on TVL
        em_data['farm_balance'] = em_data['farm_tvl'] / 2  # This is the total trunk balance in the farms
        farm_depot_tvl = 196598  # In Trunk, TODO: should be able to read this automatically
        farm_depot_claimed = 872925  # In Trunk
