...
buy_vol = df['BUY VOL'][:30].median()  # Get most recent 30 days
sell_vol = df['SELL VOL'][:30].median()
...
bertha_out_usd = df['Daily OUT.1'].apply(convert_currency_string_to_numeric).median()
bertha_in_usd = df['Daily IN.1'].apply(convert_currency_string_to_numeric).median()
...
dune_data['trunk_support_apr'] = df['balance']['PegSupport'] / avg_treasury / 30
dune_data['redemption_support_apr'] = df['balance']['Redemption'] / avg_treasury / 30

df = pd.read_csv('chain_data/bertha_in_30.csv', index_col='Date', parse_dates=True)
daily = df.pivot(columns='category', values='daily').median()
bwb_taxes = daily['BWB'] / daily.sum() * bertha_in_usd
dune_data['nft_mint_volume'] = daily['NFT Mint'] / daily.sum() * bertha_in_usd
dune_data['nft_sell_taxes'] = daily['NFT Royalties'] / daily.sum() * bertha_in_usd
buyback = daily['BuyBack'] / daily.sum() * bertha_in_usd

# Calculate various parameters
market_buy_volume = buy_vol - buyback - dune_data['nft_mint_volume']
market_sell_volume = sell_vol - bertha_out_usd
dune_data['bwb_volume'] = bwb_taxes / 0.08 * (buy_vol / total_vol)
dune_data['swb_volume'] = bwb_taxes / 0.08 * (sell_vol / total_vol)
dune_data['pcs_buy_volume'] = market_buy_volume - dune_data['bwb_volume']
dune_data['pcs_sell_volume'] = market_sell_volume - dune_data['swb_volume']
dune_data['total_buy_volume'] = buy_vol
dune_data['total_sell_volume'] = sell_vol
dune_data['%buy_bertha'] = (buyback + dune_data['nft_mint_volume']) / buy_vol * 100
dune_data['%buy_bwb'] = dune_data['bwb_volume'] / buy_vol * 100
dune_data['%buy_pcs'] = dune_data['pcs_buy_volume'] / buy_vol * 100
...
f = open('chain_data/DuneData_{0}.pkl'.format(dt.datetime.now().strftime('%Y-%m-%d %H:%M')), 'wb')
pickle.dump(dune_data, f)
f.close()
